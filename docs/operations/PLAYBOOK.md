...
**핵심 원칙**: 01~05번 에이전트는 사용자에게 직접 보고하지 않는다.
모든 보고는 06번(비서실장)을 경유하며, 06번이 우선순위를 선정하여 최종 브리핑한다.
...
Step 1. 사용자 요청 접수
    │   사용자가 작업을 지시한다.
    ↓
Step 2. 06번 비서실장 — 작업 분배
    │   요청을 분석하여 적절한 에이전트에게 할당한다.
    │   ├── UI/UX 관련 → 01번 (Frontend)
    │   ├── 백엔드 로직 → 02번 (Backend)
    │   ├── 데이터/AI → 03번 (Data & AI)
    │   ├── 품질 검증 → 04번 (QA)
    │   └── 인프라/배포 → 05번 (DevOps)
...
Step 3. 담당 에이전트 — 작업 수행
    ...
    ↓
Step 4. 담당 에이전트 → 06번 보고
    │   작업 결과, 변경 파일, 발견된 이슈를 보고한다.
    ↓
Step 5. 06번 비서실장 — 취합 및 브리핑
...
```
Step 1. 이슈 감지
    │   04번(QA) 또는 05번(DevOps)이 문제를 발견한다.
    ↓
Step 2. 즉시 수정 조치
...
Step 3. 06번에게 긴급 보고
    ...
    ↓
Step 4. 06번 → 사용자 즉시 알림
    │   비서실장이 사용자에게 긴급 브리핑을 수행한다.
    ↓
Step 5. 사후 조치
...
git commit -m "[Docs]: CLAUDE.md 행동 수칙 10번 Git 커밋 규칙 추가"
...
| 에이전트 | 직접 커밋 | 이유 |
|---------|---------|------|
| 01번 (Frontend) | ❌ 금지 | 05번 감독 필수 |
| 02번 (Backend) | ❌ 금지 | 05번 감독 필수 |
| 03번 (Data/AI) | ❌ 금지 | 05번 감독 필수 |
| 04번 (QA) | ❌ 금지 | 검수 후 05번에 전달 |
| 05번 (DevOps) | ✅ 허용 | 커밋/병합 총괄 책임자 |
| 06번 (비서실장) | ❌ 금지 | 보고 전담 역할 |
...
| 작업 유형 | 호출 에이전트 | 예시 |
|----------|-------------|------|
| UI 컴포넌트 신규/수정 | 01번 | 대시보드 카드 추가, 반응형 수정 |
| 백엔드 로직 수정 | 02번 | 서비스 함수 추가, API 연동 |
| AI 요약 품질 개선 | 03번 | 프롬프트 수정, 모델 라우팅 변경 |
| 코드 리뷰/보안 점검 | 04번 | PR 리뷰, 보안 스캔, 빌드 검증 |
| 배포/인프라 작업 | 05번 | CI/CD 수정, Lambda 설정, 장애 대응 |
| 전체 상황 브리핑 | 06번 | 진행 보고, 우선순위 정리, 승인 요청 |
