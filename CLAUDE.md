...
9. **중앙 보고 체계 준수**: 01~03번 에이전트는 직접 사용자에게 보고하지 않고,
   반드시 04번(Tech Lead PM)을 경유하여 보고한다.
   04번은 사용자가 살펴봐야 할 High Priority 항목을 선별하여 브리핑한다.
10. **Git 커밋 규칙 엄수**: 01번·03번 에이전트는 코드 수정 후 직접 커밋하지 않는다.
    반드시 02번(Backend Cloud) 에이전트가 04번(Tech Lead PM) QA 검수 후 커밋을 수행한다.
    아래 한글 커밋 메시지 템플릿을 사용하여 커밋한다.
    ```
    [Feat]: 새로운 기능 추가
    [Fix]: 버그 수정
...
4. **중앙 보고**: 모든 에이전트의 작업 결과는 04번(Tech Lead PM)이 취합하여
